...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public String modifyUserChartAvailableNum(Long userId, Integer num) {
        ...
        int newNum = user.getAvailableNum() + num;
        updateUser.setAvailableNum(newNum);
        if (updateById(updateUser)) {
            //更新用户登陆信息
            user.setAvailableNum(newNum);
            Gson gson = new Gson();
            LoginUserVO loginUserVO = getLoginUserVO(user);
            HashMap<String, String> map = new HashMap<>();
            map.put("user", gson.toJson(loginUserVO));
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        LoginUserVO result = getLoginUserVO(user);
        // 3. 记录用户的登录态
        HashMap<String, String> map = new HashMap<>();
        Gson gson = new Gson();
        map.put("user", gson.toJson(result));
    ...
    public boolean isAdmin(HttpServletRequest request) {
        // 仅管理员可查询
        String token = request.getHeader(USER_LOGIN_STATE);
        User user = JWTUtils.verifyAndGetUser(token);
        return isAdmin(user);
    }
